    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == butSquare) {
            if(backEnd.getExpression().length()==0||!Character.isDigit(backEnd.getExpression().charAt(backEnd.getExpression().length()-1))){
                JOptionPane.showMessageDialog(frame, "Cannot square a non-number!");
                return;
            }
        ...
        if (source == butExp) {
            if(backEnd.getExpression().length()==0||!Character.isDigit(backEnd.getExpression().charAt(backEnd.getExpression().length()-1))||backEnd.getExpression().length()==0){
                JOptionPane.showMessageDialog(frame, "Cannot exponentiate a non-number!");
                return;
            }
        ...
        if (source == butDivide) {
            if(backEnd.getExpression().length()==0||!Character.isDigit(backEnd.getExpression().charAt(backEnd.getExpression().length()-1))||backEnd.getExpression().length()==0){
                JOptionPane.showMessageDialog(frame, "Syntax: Number must precede division symbol");
                return;
            }
            backEnd.addToExpression("/");
        }
        if (source == butAdd) {
            if(backEnd.getExpression().length()==0||!Character.isDigit(backEnd.getExpression().charAt(backEnd.getExpression().length()-1))||backEnd.getExpression().length()==0){
                JOptionPane.showMessageDialog(frame, "Syntax: Number must precede addition symbol");
                return;
            }
            backEnd.addToExpression("+");
        }
        if (source == butMinus) {
            if(backEnd.getExpression().length()==0||!Character.isDigit(backEnd.getExpression().charAt(backEnd.getExpression().length()-1))||backEnd.getExpression().length()==0){
                JOptionPane.showMessageDialog(frame, "Syntax: Number must precede minus symbol");
                return;
            }
            backEnd.addToExpression("-");
        }
        if(source == butMultiply){
            if(backEnd.getExpression().length()==0||!Character.isDigit(backEnd.getExpression().charAt(backEnd.getExpression().length()-1))||backEnd.getExpression().length()==0){
                JOptionPane.showMessageDialog(frame, "Syntax: Number must precede multiplication symbol");
                return;
            }
            backEnd.addToExpression("*");
        }
        ...
        if(source == butDot){
            if(backEnd.getExpression().length()==0||!Character.isDigit(backEnd.getExpression().charAt(backEnd.getExpression().length()-1))||backEnd.getExpression().length()==0){
                JOptionPane.showMessageDialog(frame, "Syntax: Number must precede a dot");
                return;
            }
            backEnd.addToExpression(".");
        }
